...
template<typename InterfaceT, typename StateT>
class fsm_ctx
{
	std::unordered_map<size_t, char *> buffers;
	StateT state_;
	InterfaceT *current_interface;
	size_t current_interface_size_as_key;
	friend class fsm<InterfaceT>;
	friend class impl_ctx<InterfaceT, StateT>;
public:
	fsm_ctx(StateT state)
		: state_(state)
		, current_interface(nullptr)
		, current_interface_size_as_key(0)
	{}
	fsm_ctx()
		: state_()
		, current_interface(nullptr)
		, current_interface_size_as_key(0)
	{}
	fsm_ctx &operator =(const fsm_ctx &rhs)
	{
		if (this != &rhs)
		{
			state_ = rhs.state_;
			//The rest won't copy until second phase of initialization
			current_interface = nullptr;
			current_interface_size_as_key = 0;
		}
		return *this;
	}
	fsm_ctx(const fsm_ctx &rhs)
		: state_(rhs.state_)
		//The rest won't copy until second phase of initialization
		, current_interface(nullptr)
		, current_interface_size_as_key(0)
...
public:
	...
	typedef fsm_ctx<InterfaceT, StateT> fsm_ctx_t;
	typedef impl_ctx *impl_ctx_ptr;
	friend class fsm<InterfaceT>;

private:
	fsm_ctx_t *fsm_ctx_;
protected:
	virtual InterfaceT *clone_interface(fsm_ctx_t *afsm_ctx) const =0;

	template <typename U>
	static U *create_new_interface(U u, fsm_ctx_t *afsm_ctx)
	{
		//placement new, see:
		//http://www.drdobbs.com/cpp/calling-constructors-with-placement-new/232901023

		auto alloc_mem_buf_it = afsm_ctx->buffers.find(sizeof(U));
		char *mem_buf = nullptr;
		size_t current_interface_size_as_key = sizeof(U);

		if (alloc_mem_buf_it == afsm_ctx->buffers.end())
		{
			mem_buf = new char[current_interface_size_as_key];
			afsm_ctx->buffers[current_interface_size_as_key] = mem_buf;
		}
		else
		{
			mem_buf = alloc_mem_buf_it->second;
		}

		U *new_interface = (U *) new (mem_buf) U();

		afsm_ctx->current_interface = new_interface;
		afsm_ctx->current_interface_size_as_key = current_interface_size_as_key;
		new_interface->fsm_ctx_ = afsm_ctx;
...
public:
	...
	{
		this->~impl_ctx();
		return create_new_interface(U(), fsm_ctx_);
	}
	state_t &state()
	{
		return fsm_ctx_->state_;
	}
};

template <typename StateImplementationT, typename InterfaceT>
struct impl_state : public InterfaceT
{
	typedef typename InterfaceT::fsm_ctx_t fsm_ctx_t;
	InterfaceT *clone_interface(fsm_ctx_t *afsm_ctx) const override
	{
		return InterfaceT::create_new_interface(StateImplementationT(), afsm_ctx);
	}
...
class fsm
{
	typedef typename InterfaceT::state_t state_t;
	typedef typename InterfaceT::fsm_ctx_t fsm_ctx_t;
	typedef typename InterfaceT::impl_ctx_ptr impl_ctx_ptr_t;
	fsm_ctx_t fsm_ctx_;

public:
	struct uninitialized : public std::runtime_error
	{
	...
	fsm() = default;
	fsm(const state_t &state) : fsm_ctx_(state) {}
	fsm &operator=(const fsm &rhs)
	{
		if (this != &rhs)
		{
			fsm_ctx_ = rhs.fsm_ctx_;
			//second phase intialization
			rhs.curr_interface()->clone_interface(&fsm_ctx_);
		}
		return *this;
	}
	fsm(const fsm &rhs)
	...
	U *move_to_state()
	{
		auto alloc_mem_buf_it = fsm_ctx_.buffers.find(sizeof(U));
		if (alloc_mem_buf_it != fsm_ctx_.buffers.end())
			static_cast<impl_ctx_ptr_t>(curr_interface())->~impl_ctx();
		U *new_iface = InterfaceT::create_new_interface(U(), &fsm_ctx_);

		return new_iface;
	}
	InterfaceT *curr_interface()
	{
		return reinterpret_cast<InterfaceT *>(fsm_ctx_.current_interface);
	}
	InterfaceT *curr_interface() const
	{
		return reinterpret_cast<InterfaceT *>(fsm_ctx_.current_interface);
	}
	...
	InterfaceT *operator ->()
	{
        std::string err_msg = std::string("exception! uninitialized interface: ") + __FUNCTION__;
        if (!curr_interface())
			throw uninitialized(err_msg);
	...
	~fsm()
	{
		if (curr_interface())
		{
			auto alloc_mem_buf_it = fsm_ctx_.buffers.find(fsm_ctx_.current_interface_size_as_key);
			if (alloc_mem_buf_it != fsm_ctx_.buffers.end())
			{
				static_cast<impl_ctx_ptr_t>(curr_interface())->~impl_ctx();
			}
			for (auto &it : fsm_ctx_.buffers)
				delete[] it.second;
		}
	}
};
